...
class GnomeShellBackend(GUIBackend):
    ...
    def _display_config(self):
        ...
        if self._display_proxy is None:
            try:
                self._display_proxy = displayconfig.new_proxy()
            except Exception as exc:
                raise CapabilityUnsupported(
                    Capability.SCREEN_INFO,
                    self.name,
                    f"cannot reach {displayconfig.BUS_NAME}: {exc}",
                ) from exc
    ...
    def screens(self):
        ...
        Read from Mutter's own `DisplayConfig.GetCurrentState` -- see the
        displayconfig module, which owns the call and the arithmetic behind
        it -- so this is the one method here that needs nothing from the
        extension, and the only source in this package that reports a
        *scale*. X11 has no notion of one, which is why an XWayland
        session's outputs come back at 1.0 however the desktop is actually
        scaled.

        Logical monitors, not physical ones, so a mirrored pair is one
        Screen and a disabled output is none at all; sizes are derived
        from the panel's mode, the scale and the transform. Both are the
        shared function's business, and its docstring is where the reasons
        are written down.

        The extension is still what gets this backend *constructed* (see
        `__init__`), so a GNOME session without it gets no SCREEN_INFO
        here either -- an artefact of where this method lives rather than
        of what Mutter allows. A caller who wants outputs without window
        control can call the module directly, which is what `_screen_size`
        does.
        """
        self.require(Capability.SCREEN_INFO)
        try:
            state = displayconfig.read(self._display_config())
        except Exception as exc:
            raise CapabilityUnsupported(
                Capability.SCREEN_INFO,
                self.name,
        ...
        return [
            Screen(index=index, width=width, height=height, scale=scale, name=name)
            for index, (_x, _y, width, height, scale, name) in enumerate(
