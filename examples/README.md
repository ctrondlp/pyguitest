...
| `_inputcapture_validate.py` | Not numbered: the first live check of `Capability.INPUT_CAPTURE`/`Session.wait_for_pointer_activation()`, unverified against a real portal until this script runs it. **Read its own docstring before running** -- unlike every other script here, *approving* the consent dialog exclusively diverts your own pointer away from your desktop for as long as the capture stays active, so this is not something to run unattended or from an agent session. Reports the zones and pointer barriers computed, then waits for you to move your pointer to a screen edge and prints the position and timing the compositor handed back. Candidate for removal or promotion to a real numbered example once it has actually been run. |
